...
def decorr_no_eq(data):
    gIn = pm.timeseries.statistical_inefficiency(data,fast=args.fast)
    indices = pm.timeseries.subsample_correlated_data(data, g=gIn,fast=args.fast)
    data_uncorr = data[indices]
    return(data_uncorr, gIn)

def decorr_eq(data):
    [t0, g, Neff_max] = pm.timeseries.detect_equilibration(data,fast=args.fast) # compute indices of uncorrelated timeseries
    A_t_equil = data[t0:]
    indices = pm.timeseries.subsample_correlated_data(A_t_equil, g=g,fast=args.fast)
