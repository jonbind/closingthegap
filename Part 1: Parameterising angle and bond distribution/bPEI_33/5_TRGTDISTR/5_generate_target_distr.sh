...
while [ $IBOND -lt $NBONDS ]; do
    echo $IBOND | gmx distance -f ../3_MAPTRAJ/mapped.xtc -n bonds.ndx -s ../4_CGINIT/CG.tpr -oall $DIR/bond_$IBOND.xvg -xvg none &> $DIR/temp_bonds.txt
...
while [ $IANG -lt $NANGLES ]; do
    echo $IANG | gmx angle -f ../3_MAPTRAJ/mapped.xtc -n angles.ndx -ov $DIR/ang_$IANG.xvg -od $DIR/temp.xvg &> $DIR/temp_angles.txt
